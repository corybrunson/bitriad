...
# Hobson, "The Evolution of Modern Capitalism", p. 271
# Read data directly into bipartite incidence matrix
southafrica1905 <- graph_from_incidence_matrix(
    as.matrix(read.csv("Hobson-directors.csv",
                       row.names = 1)))
...
if(file.exists("../data")) {
    for(name in c(
        "southafrica1905",  # small
        "women_group",  # widely used, dynamic, triadic example
        "women_clique",  # triadic example
        "scotland1920s",  # multiple components
        "minneapolis1970s",  # widely used, triadic example
        "chicago1960s",  # why not
        "nmt_meetings",  # partial; contrast w/ organizations
        "nmt_organizations",  # partial; contrast w/ meetings
        "whigs"  # high actor-event ratio; contrast w/ noordin
    )) {
