...
local.c.df <- cbind(C = local.c,
                    C.O = opsahl.transitivity(DDGG2, type = 'local'),
...
```{r}
global.c2 <- c(transitivity(DDGG2.proj),
               opsahl.transitivity(DDGG2),
               excl.transitivity(DDGG2))
...
```{r}
ddc <- data.frame(k = degree(DDGG2.proj),
...
```{r, echo = FALSE}
set.seed(1)
plot(DDGG1, layout = layout.fruchterman.reingold(DDGG1, niter = 100),
     vertex.color = ifelse(V(DDGG1)$type == 0, 'SkyBlue2', 'lightcoral'),
     vertex.shape = ifelse(V(DDGG1)$type == 0, 'circle', 'square'),
     edge.width = 2, edge.color = 'black',
     vertex.label = substr(V(DDGG1)$name, 1, 2),
     vertex.label.family = 'sans', vertex.label.color = 'white')
```
...
ddc2 <- data.frame(k = degree(DDGG1.proj),
                   C = transitivity(DDGG1.proj, type = 'local'))
print(ddc2)
...
plot(aggregate(DDGG1.wedges$C, by = list(V = DDGG1.wedges$V), FUN = mean),
     pch = 19, type = 'b',
