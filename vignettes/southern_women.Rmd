...
```{r}
ddgg.clique.proj <- actor.projection(ddgg.clique)
tc <- simple.triad.census(ddgg.clique.proj)
tc
```
...
```{r}
ftc <- an.triad.census(ddgg.clique)
ftc
```
...
We can sacrifice information about event multiplicity within triads for a simpler and more intuitive layout. This alternative is dubbed the **structural triad census** because it collapses the events to only four types, based on which actors cooperated on them, or coattended them:
...
local.c.df <- cbind(C = local.c,
                    C.opsahl = opsahl.transitivity(ddgg.clique, type = 'local'),
                    C.excl = excl.transitivity(ddgg.clique, type = 'local'))
...
```{r}
ddgg.group.proj <- actor.projection(ddgg.group)
