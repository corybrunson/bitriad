...
The function `plot.an` uses shape and color to distinguish between actor and event nodes, following what i get the impression is the reigning convention. The function defaults to the Fruchterman-Reingold algorithm. (Note: My naming convention for several functions mimics that for a hypothetical class `an` (for "affiliation network"), which may be appropriate for this package but is not currently defined.)
...
```{r}
stc <- census.proj$structural
3 * sum(stc[3, ]) / (sum(stc[2, ]) + 3 * sum(stc[3, ]))
```
...
```{r}
exclWedges <- transitivity.an(ddggs.clique, type = "")
exclWedges
```
